...
from models.audiotransformer import ced_tiny
...
class CedEncoder(torch.nn.Module):
    def __init__(self, checkpoint_path: str | None = None):
        ...
        # Default checkpoint for tiny (updated path)
        default_ckpt = '/data1/repos/EAT_projs/checkpoints/weights/audiotransformer_tiny_mae_as_10s.pt'
        use_ckpt = checkpoint_path or (default_ckpt if os.path.isfile(default_ckpt) else None)

        self.model = ced_tiny(pretrained=use_ckpt is None)
        self.output_dim = 192

        if use_ckpt is not None:
            print(f"Loading CED tiny model from: {use_ckpt}")
...
if __name__ == "__main__":
    ...
    print("Testing CED tiny encoder...")
    encoder = CedEncoder()
    if check_audio_encoder(encoder):
        print("✓ CED tiny encoder passed validation")
    else:
        print("✗ CED tiny encoder failed validation")