...
from models.audiotransformer import ced_mini
...
class CedEncoder(torch.nn.Module):
    def __init__(self, checkpoint_path: str | None = None):
        ...
        # Default checkpoint for mini (updated path)
        default_ckpt = '/data1/repos/EAT_projs/checkpoints/weights/audiotransformer_mini_mae_as_10s.pt'
        use_ckpt = checkpoint_path or (default_ckpt if os.path.isfile(default_ckpt) else None)

        self.model = ced_mini(pretrained=use_ckpt is None)
        self.output_dim = 256

        if use_ckpt is not None:
            print(f"Loading CED mini model from: {use_ckpt}")
...
if __name__ == "__main__":
    ...
    print("Testing CED mini encoder...")
    encoder = CedEncoder()
    if check_audio_encoder(encoder):
        print("✓ CED mini encoder passed validation")
    else:
        print("✗ CED mini encoder failed validation")