...
class DynamicPositionalEncoder(nn.Module):
    ...
    def forward(self, x, padding_mask):
        ...
        # Calculate patch dimensions dynamically
        # Assuming square patches for simplicity
        num_patches_h = int(num_patches ** 0.5)
        num_patches_w = num_patches // num_patches_h
        
        # Generate 2D positional embeddings on-the-fly
        pos_emb = get_2d_sincos_pos_embed_flexible(
            embed_dim,
        ...
        pos_emb = torch.from_numpy(pos_emb).float().unsqueeze(0).expand(B, -1, -1)
