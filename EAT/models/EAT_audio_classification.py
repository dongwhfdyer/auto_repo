...
from ..tasks.finetuning import MaeImageClassificationConfig
...
class MaeImageClassificationModel(BaseFairseqModel):
    def __init__(self, cfg: MaeImageClassificationConfig):
        ...
        # Clean pretrained task config by removing unknown parameters
        current_task_dc = MaeImageClassificationConfig()
        current_task_fields = set(current_task_dc.__dataclass_fields__.keys())
        
        with open_dict(pretrained_args.task):
            # Remove any fields that don't exist in current task config
            for field_name in list(pretrained_args.task.keys()):
                if field_name not in current_task_fields:
                    logger.warning(f"Removing unknown task parameter: {field_name}")
                    del pretrained_args.task[field_name]
    ...
    def forward(
        ...
        if not self.audio_mae or (self.audio_mae and (self.esc50_eval or self.spcv2_eval )):
            ...
            if not self.training:
                with torch.no_grad():
                    ...
                    correct = (pred == labels_argmax).sum()
        ...
        # Always add predictions and targets for evaluation (but don't log through criterion)
            if not self.training:
