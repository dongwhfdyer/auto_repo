...
class MaeImageClassificationTask(MaeImagePretrainingTask):
    ...
    def valid_step(self, sample, model, criterion):
        # Use parent's valid_step for proper model state and error handling
        loss, sample_size, logging_output = super().valid_step(sample, model, criterion)
        
        # Get predictions from the model's forward pass that already happened in criterion
        # Access the stored net_output from the criterion to avoid double computation
        if hasattr(criterion, 'last_net_output'):
            net_output = criterion.last_net_output
            if "_predictions" in net_output and "_targets" in net_output:
                predictions = net_output["_predictions"]
                targets = net_output["_targets"]
                
                # Store for reduce_metrics
                if not hasattr(self, '_eval_predictions_list'):
                    self._eval_predictions_list = []
                    self._eval_targets_list = []
                
                self._eval_predictions_list.append(predictions.cpu())
                self._eval_targets_list.append(targets.cpu())
    ...
    def reduce_metrics(self, logging_outputs, criterion):
        ...
        if "correct" in logging_outputs[0]:
        ...
        elif hasattr(self, '_eval_predictions_list') and len(self._eval_predictions_list) > 0:
            # Use collected predictions and targets from valid_step
            all_predictions = torch.cat(self._eval_predictions_list, dim=0)
            all_targets = torch.cat(self._eval_targets_list, dim=0)
            
            # Log for metrics calculation
            metrics.log_concat_tensor("_predictions", all_predictions)
            metrics.log_concat_tensor("_targets", all_targets)
            ...
            # Clear the lists for next validation
            self._eval_predictions_list = []
            self._eval_targets_list = []
