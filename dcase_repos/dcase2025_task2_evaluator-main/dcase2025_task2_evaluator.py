...
OFFICIAL_SCORE_COLUMNS = [
    ...
    *[f"{machine_type} AUC (source)" for machine_type in MACHINE_TYPE_LIST],
    *[f"{machine_type} AUC (target)" for machine_type in MACHINE_TYPE_LIST],
]
...
SCORE_COLUMNS = [
    "official score", "arithmetic mean", "harmonic mean (source)", "harmonic mean (target)",
    *[f"{machine_type} Score (source)" for machine_type in MACHINE_TYPE_LIST],
    *[f"{machine_type} Score (target)" for machine_type in MACHINE_TYPE_LIST],
]
PAPER_OFFICIAL_SCORE_COLUMNS = ["h-mean", "a-mean", *MACHINE_TYPE_LIST]
    ...
    for machine_idx, target_machine in enumerate(machines):
        ...
        for section_id in section_ids:
            ...
            if not os.path.exists(anomaly_score_path) or \
                ...
                # Load DCASE2025 baseline results
                anomaly_score_path = "{dir}/anomaly_score_DCASE2025T2{machine}_{section}_test_seed{seed}{tag}_Eval.csv".format(
                    dir=target_dir,
                    machine=target_machine,
                    section=section_id,
                    seed=seed,
                    tag=tag)
                decision_result_path = "{dir}/decision_result_DCASE2025T2{machine}_{section}_test_seed{seed}{tag}_Eval.csv".format(
                    dir=target_dir,
                    machine=target_machine,
                    section=section_id,
                    seed=seed,
            ...
            if not os.path.exists(anomaly_score_path) or \
                ...
                print(f"\tanomaly_score_path : {os.path.exists(anomaly_score_path)}")
                print(f"\tdecision_result_path : {os.path.exists(decision_result_path)}")
