...
def train(
    ...
    # Track last train_inner loss (windowed) to use for checkpointing when no validation
    last_train_inner_loss = None
    ...
    for i, samples in enumerate(progress):
        ...
        # Refresh last_train_inner_loss from current train_inner meters (if available)
        m = metrics.get_meter("train_inner", "loss")
        if m is not None and m.avg is not None:
            last_train_inner_loss = m.avg
        ...
        valid_losses, should_stop = validate_and_save(
            cfg, trainer, task, epoch_itr, valid_subsets, end_of_epoch, last_train_inner_loss
        )
...
def validate_and_save(
    ...
    end_of_epoch: bool,
    last_train_inner_loss: Optional[float],
) -> Tuple[List[Optional[float]], bool]:
    num_updates = trainer.get_num_updates()
    ...
    if do_save or should_stop:
        cp_path = checkpoint_utils.save_checkpoint(
            cfg.checkpoint, trainer, epoch_itr, valid_losses[0], last_train_inner_loss
        )
