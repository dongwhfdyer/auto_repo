...
def save_checkpoint(cfg: CheckpointConfig, trainer, epoch_itr, val_loss, train_inner_loss: Optional[float] = None):
    ...
    # Defer training loss retrieval until after we know whether we're at end of epoch
    train_loss = None
    loss_for_checkpoint = val_loss
    ...
    # Compute loss_for_checkpoint (prefer validation; else use provided train_inner loss)
    if loss_for_checkpoint is None:
        if train_inner_loss is not None:
            loss_for_checkpoint = train_inner_loss
        else:
            from fairseq.logging import metrics
            train_loss_meter = metrics.get_meter("train_inner", "loss")
            if train_loss_meter is not None:
                train_loss = train_loss_meter.avg
                loss_for_checkpoint = train_loss
    ...
    # Track and update best
    prev_best = getattr(save_checkpoint, "best", None)
    if loss_for_checkpoint is not None:
        best_function = max if cfg.maximize_best_checkpoint_metric else min
        save_checkpoint.best = (
            loss_for_checkpoint if prev_best is None else best_function(loss_for_checkpoint, prev_best)
        )

    # Check if this is a new best checkpoint (non-strict, allow ties)
    is_new_best = (
        loss_for_checkpoint is not None
        and (prev_best is None or is_better(loss_for_checkpoint, prev_best))
    )

    checkpoint_conds["checkpoint_best{}.pt".format(suffix)] = is_new_best

    # Log best checkpoint information
    if is_new_best:
        metric_name = cfg.best_checkpoint_metric
        current_metric = loss_for_checkpoint
        previous_best_val = prev_best
        best_file = os.path.join(cfg.save_dir, f"checkpoint_best{suffix}.pt")

        prev_str = "None" if previous_best_val is None else f"{previous_best_val:.4f}"
        logger.info(
            f"🎯 NEW BEST CHECKPOINT SAVED | "
            f"Metric: {metric_name} | "
            f"Previous best: {prev_str} | "
            f"Current: {current_metric:.4f} | "
            f"File: {best_file}"
        )
    if loss_for_checkpoint is not None and cfg.keep_best_checkpoints > 0:
        ...
        with data_utils.numpy_seed(epoch, updates, loss_for_checkpoint):
            rand_sfx = np.random.randint(0, cfg.keep_best_checkpoints)

        checkpoint_conds[
            "checkpoint.best_{}_{:.3f}{}{}.pt".format(
                cfg.best_checkpoint_metric, loss_for_checkpoint, rand_sfx, suffix
            )
        ] = worst_best is None or is_better(loss_for_checkpoint, worst_best)
    checkpoint_conds[
