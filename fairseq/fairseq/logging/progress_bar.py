...
def progress_bar(
    ...
    # Add debug: show whether tensorboard is enabled and current cwd
    if tensorboard_logdir:
        logger.info(f"[progress_bar] tensorboard_logdir={tensorboard_logdir} cwd={os.getcwd()}")
        try:
            # [FB only] custom wrapper for TensorBoard
            import palaas  # noqa
            from .fb_tbmf_wrapper import FbTbmfWrapper
            bar = FbTbmfWrapper(bar, log_interval)
            logger.info("[progress_bar] using FbTbmfWrapper for TensorBoard")
        except ImportError:
            logger.info("[progress_bar] using TensorboardProgressBarWrapper")
            bar = TensorboardProgressBarWrapper(bar, tensorboard_logdir)
    else:
        logger.info("[progress_bar] TensorBoard disabled (no logdir)")
...
try:
    ...
    logger.info("[tb] using torch.utils.tensorboard SummaryWriter")
except ImportError:
    try:
        from tensorboardX import SummaryWriter
        logger.info("[tb] using tensorboardX SummaryWriter")
    except ImportError:
        SummaryWriter = None
        logger.warning("[tb] no SummaryWriter available (install tensorboard or tensorboardX)")
...
class TensorboardProgressBarWrapper(BaseProgressBar):
    ...
    def __init__(self, wrapped_bar, tensorboard_logdir):
        ...
        if SummaryWriter is None:
            ...
            logger.info(f"[tb] wrapper init: logdir={self.tensorboard_logdir}")

    def _writer(self, key):
        if SummaryWriter is None:
            logger.warning(f"[tb] _writer called with key={key} but SummaryWriter is None")
            return None
        _writers = _tensorboard_writers
        if key not in _writers:
            path = os.path.join(self.tensorboard_logdir, key)
            logger.info(f"[tb] creating SummaryWriter at {path}")
            try:
                _writers[key] = SummaryWriter(path)
                _writers[key].add_text("sys.argv", " ".join(sys.argv))
            except Exception as e:
                logger.exception(f"[tb] failed to create SummaryWriter at {path}: {e}")
                return None
        return _writers[key]
    ...
    def log(self, stats, tag=None, step=None):
        ...
        logger.info(f"[tb] log tag={tag} step={step} keys={list(stats.keys())}")
        self._log_to_tensorboard(stats, tag, step)
        self.wrapped_bar.log(stats, tag=tag, step=step)

    def print(self, stats, tag=None, step=None):
        """Print end-of-epoch stats."""
        logger.info(f"[tb] print tag={tag} step={step} keys={list(stats.keys())}")
        self._log_to_tensorboard(stats, tag, step)
        self.wrapped_bar.print(stats, tag=tag, step=step)
    ...
    def _log_to_tensorboard(self, stats, tag=None, step=None):
        ...
        if writer is None:
            logger.warning(f"[tb] writer is None for tag={tag}")
            return
        if step is None:
            if "num_updates" in stats:
                step = stats["num_updates"]
            else:
                logger.warning("[tb] step missing and 'num_updates' not in stats; defaulting to 0")
                step = 0
        for key in stats.keys() - {"num_updates"}:
            try:
                if isinstance(stats[key], AverageMeter):
                    ...
                    logger.info(f"[tb] add_scalar tag={tag or ''} key={key} value={value} step={step}")
                ...
                logger.exception(f"[tb] failed to add_scalar key={key} step={step}: {e}")
        try:
            writer.flush()
        except Exception as e:
            logger.exception(f"[tb] failed to flush writer: {e}")
